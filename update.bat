...
echo Map: %~dp0
echo.
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; try { $dest='%~dp0'; $zip=Join-Path $env:TEMP 'qwp-update.zip'; $tmp=Join-Path $env:TEMP 'qwp-update'; Write-Host 'Downloaden van GitHub...'; Invoke-WebRequest -Uri 'https://github.com/RareGoudvis/questi-calendar-workflow-fix/archive/refs/heads/main.zip' -OutFile $zip; if (Test-Path $tmp) { Remove-Item $tmp -Recurse -Force }; Expand-Archive -Path $zip -DestinationPath $tmp -Force; $src=Join-Path $tmp 'questi-calendar-workflow-fix-main'; if (-not (Test-Path $src)) { throw 'Uitgepakte map niet gevonden.' }; Write-Host 'Bestanden vervangen...'; Copy-Item -Path (Join-Path $src '*') -Destination $dest -Recurse -Force; Remove-Item $zip -Force; Remove-Item $tmp -Recurse -Force; Write-Host 'OK.' } catch { Write-Host ('FOUT: ' + $_.Exception.Message) -ForegroundColor Red; exit 1 }"
if errorlevel 1 (
  echo.
